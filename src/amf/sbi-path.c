...
int amf_sbi_open(void)
{
    ...
    if (ogs_sbi_nf_service_is_available(OGS_SBI_SERVICE_NAME_NAMF_MT)) {
        service = ogs_sbi_nf_service_build_default(
                    nf_instance, OGS_SBI_SERVICE_NAME_NAMF_MT);
        ogs_assert(service);
        ogs_sbi_nf_service_add_version(
                    service, OGS_SBI_API_V1, OGS_SBI_API_V1_0_0, NULL);
        ogs_sbi_nf_service_add_allowed_nf_type(service, OpenAPI_nf_type_SMSF);
...
void amf_smsf_sms_send_uplink_sms(amf_ue_t *amf_ue, void *data)
{
    ...
    xact = ogs_sbi_xact_add(
            ...
            OGS_SBI_SERVICE_TYPE_NSMSF_SMS, discovery_option,
