...
smsf_ue_t *smsf_ue_add(char *supi)
{
    ...
    ogs_pool_id_calloc(&smsf_ue_pool, &smsf_ue);
    if (!smsf_ue) {
    ...
    if (!smsf_ue->ctx_id) {
        ...
        ogs_pool_id_free(&smsf_ue_pool, smsf_ue);
        return NULL;
    }
...
smsf_ue_t *smsf_ue_find_by_id(ogs_pool_id_t id)
{
    return ogs_pool_find_by_id(&smsf_ue_pool, id);
}
